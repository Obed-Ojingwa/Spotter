...
// z.number() + valueAsNumber: true on the input — RHF converts string→number
// before Zod sees it, keeping TypeScript happy with useForm<FormData>.
...
export default function AdminPromotionsPage() {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors, isSubmitting } } =
    useForm<FormData>({
      ...
      defaultValues: { reason: "weekly_promotion" },
    });
  ...
  async function onSubmit(data: FormData) {
    try {
      ...
      setLastGrant({ points: data.points, count: data.agent_id?.trim() ? 1 : 99 });
      reset({ reason: "weekly_promotion" });
    } catch (err: any) {
        ...
        <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="card space-y-5">
            ...
            <div>
              ...
              {/* valueAsNumber: true — RHF converts the HTML number input string to a JS
                  number before validation, so z.number() receives an actual number. */}
              <input
                type="number"
                step="0.5"
                min="0.5"
